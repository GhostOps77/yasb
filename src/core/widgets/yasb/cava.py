...
class CavaBar(QFrame):
    def __init__(self, cava_widget):
        ...
        bar_spacing = 0
        if self._cava_widget._bar_type in ("bars_mirrored", "bars"):
            bar_spacing = self._cava_widget._bar_spacing

        cava_widget_width = self._cava_widget._bars_number * (self._cava_widget._bar_width + bar_spacing)

        self.setFixedWidth(cava_widget_width)
    ...
    def draw_bars_mirrored(self, painter):
        ...
        for i, sample in enumerate(samples):
            ...
            if up_px > 0:
                ...
                painter.fillRect(
                    QRectF(ux_px / dpr, uy_px / dpr, band_w_px / dpr, up_px / dpr),
                    brush_upper,
                )
...
class CavaWidget(BaseWidget):
    ...
    def initialize_colors(self) -> None:
        ...
        if self._gradient == 1:
            for color_str in [
                self._gradient_color_1,
                self._gradient_color_2,
                self._gradient_color_3,
            ]:
                if not color_str:
                    continue
                try:
    ...
    def start_cava(self) -> None:
        ...
        config_template = (
            ...
            "[general]"
            f"bars = {self._bars_number}"
            f"bar_spacing = {self._bar_spacing}"
            f"bar_width = {self._bar_width}"
            f"sleep_timer = {self._sleep_timer}"
            f"sensitivity = {self._sensitivity}"
            f"lower_cutoff_freq = {self._lower_cutoff_freq}"
            f"higher_cutoff_freq = {self._higher_cutoff_freq}"
            f"framerate = {self._framerate}"
            f"noise_reduction = {self._noise_reduction}"
            ""
            "[output]"
            "method = raw"
            f"bit_format = {self._output_bit_format}"
            f"orientation = {self._orientation}"
            f"channels = {self._channels}"
            f"mono_option = {self._mono_option}"
            f"reverse = {self._reverse}"
            f"waveform = {self._waveform}"
            ""
            "[color]"
            f"foreground = '{self._foreground}'"
            f"gradient = {self._gradient}"
        )

        gradients = []
        for i in range(1, 3):
            val = getattr(self, f"_gradient_color_{i}", None)
            if val is not None:
                gradients.append(f"gradient_color_{i} = '{val}'")

        config_template += "\n".join(gradients) + "\n"
        config_template += f"[smoothing]monstercat = {self._monstercat}waves = {self._waves}"
