...
class RecycleBinWidget(BaseWidget):
    ...
    def _update_label(self):
        ...
        item_size_in_human_readable_format = naturalsize(self._bin_info["size_bytes"], binary=True, format="%.2f")

        active_label_content = active_label_content.format(
            items_count=self._bin_info["num_items"],
            items_size=item_size_in_human_readable_format,
        ...
        if self._tooltip:
            set_tooltip(
                ...
                f"Items: {self._bin_info['num_items']} ({item_size_in_human_readable_format})",
            )
