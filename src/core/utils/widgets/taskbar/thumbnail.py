...
class PreviewPopup(QFrame):
    ...
    def _calculate_and_position_popup(self):
        ...
        x = max(
            self._screen_geom.left() + self._margin,
            min(self._screen_geom.right() - popup_w - self._margin, x),
        )
        y = max(
            self._screen_geom.top() + self._margin,
            min(self._screen_geom.bottom() - total_h - self._margin, y),
        )
    ...
    def _get_source_dimensions(self):
        ...
        if hasattr(self, "_src_hwnd"):
            try:
                ...
                logger.debug(
                    "GetWindowRect failed for hwnd %s",
                    getattr(self, "_src_hwnd", None),
                    exc_info=True,
                )
...
class TaskbarThumbnailManager:
    ...
    def _show_external_thumbnail(self, src_hwnd: int, preview_popup: PreviewPopup):  # type: ignore[name-defined]
        ...
        try:
            ...
            hole_region = QRegion(
                preview_popup._thumb_local_rect.x(),
                preview_popup._thumb_local_rect.y(),
                thumb_w,
                thumb_h,
            )
